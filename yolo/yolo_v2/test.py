...
from yolo.yolo_v2.yolo_v2 import space_to_depth_x2, space_to_depth_x2_output_shape
...
class DarkNetToKeras:
    ...
    _WEIGHTS = "yolo.weights"
    _CONFIG = "yolo.cfg"
    _converted_model = "yolo_v2_converted"
    _directory_model = "model_data"

    def __init__(self):
        """

        """
        if not os.path.exists(self._directory_model):
            os.makedirs(self._directory_model)
    ...
    def _download(url, namefile):
        ...
        with open(save_file, "wb") as f:
            for chunk in tqdm(r.iter_content(chunk_size=chunksize), total=bars, unit="kB", ncols=79,
